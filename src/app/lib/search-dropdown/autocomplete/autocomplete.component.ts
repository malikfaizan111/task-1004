 ...
 * Keyboard events
 */
const isArrowUp = (keyCode : any) => keyCode == 38;
const isArrowDown = (keyCode : any) => keyCode == 40;
const isArrowUpDown = (keyCode : any) => isArrowUp(keyCode) || isArrowDown(keyCode);
const isEnter = (keyCode : any) => keyCode == 13;
const isBackspace = (keyCode : any) => keyCode == 8;
const isDelete = (keyCode : any) => keyCode == 46;
const isESC = (keyCode : any) => keyCode == 27;
const isTab = (keyCode : any) => keyCode == 9;
...
export class AutocompleteComponent implements OnInit, OnChanges, AfterViewInit, ControlValueAccessor
{
	...
	inputKeyUp$: Observable<any> = new Observable; // input events
	inputKeyDown$: Observable<any> = new Observable; // input events
	...
	private manualOpen : any= undefined;
	private manualClose : any= undefined;
	...
	@Input() public debounceTime: number = 400; // delay time while typing
	...
	public filterList()
	{
		...
		if (this.query != null && this.data)
		{
			...
			this.filteredList = this.data.filter((item: any) =>
			{
				...
				{
					if (typeof item[this.searchKeyword] == 'number')
					{
						if (item[this.searchKeyword] == parseInt(this.query))
						{
							return item;
						}
						else
	...
	public select(item : any)
	{
		...
		if (this.initialValue)
		{
			...
			if (history)
			{
				...
				if (!existingHistory.some((existingItem : any) => !this.isType(existingItem)
