...
@Component({
	selector: 'app-delivery_playlist',
	templateUrl: './delivery_playlist.component.html',
	styleUrls: ['./delivery_playlist.component.css']
})
export class DeliveryPlaylistComponent implements OnInit {
	search: string;
	...
	ngOnInit() {
		...
		if (this.componentSettings) {
			if (this.componentSettings.name != null && this.componentSettings.name == 'DeliveryPlaylist') {
	...
	addNew() {
		this.router.navigateByUrl('main/delivery_playlist/add');
	}
	// addNew1() 
	// {
	// 	let arr = [];
	...
	onEditCategorie(Playlist: any): void {
		localStorage.setItem('DeliveryPlaylist', JSON.stringify(Playlist));
		this.router.navigateByUrl('main/delivery_playlist/' + Playlist.id);
	}
}