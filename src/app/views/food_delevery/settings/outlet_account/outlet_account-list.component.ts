...
@Component({
	selector: 'app-outlet_account-list',
	templateUrl: './outlet_account-list.component.html'
})
export class OutletAccountListComponent implements OnInit {
	search: string;
	// type: any;
	...
	addNew() {
		this.router.navigateByUrl('main/outlet_account/add');
	}
	...
	getMerchantAccountList(index: any, isLoaderHidden?: boolean): void {
		...
		let url = 'viewUserForOutlet?page=' + index + '&per_page=' + this.perPage + '&sort_order=Desc';
		...
		this.mainApiService.getList(appConfig.base_url_slug + url, false, 2)
			.then(result => {
				...
				if (result.status == 200 && result.data) {
					this.merchantAccount = result.data;
					this.merchantAccountCount = result.data.pagination;
	...
	// onEditAccount(outletAccount): void
	// {
	// 	localStorage.setItem('Account', JSON.stringify(outletAccount));
	// 	this.router.navigateByUrl('main/outlet_account/add' + outletAccount.id);
	// }
	onDelete(parentOutlet: any): void {
		let url = 'deleteUserForOutlet/' + parentOutlet.id + '?';
	...
	onEditAdmin(outletAccount: any): void {
		localStorage.setItem('OutletAccount', JSON.stringify(outletAccount));
		this.router.navigateByUrl('main/outlet_account/' + outletAccount.id);
	}
