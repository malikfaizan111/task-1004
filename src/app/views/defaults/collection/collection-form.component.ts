...
export class CollectionFormComponent implements OnInit {
	...
	constructor(protected router: Router,
		...
		this.Form = this.formbuilder.group({
			...
			for_delivery: [null, [Validators.required]],
	...
	ngOnInit() {
		this.sub = this._route.params.subscribe(params => {
			...
			if (this.id != 'add') {
				...
				let abc = localStorage.getItem('Collection') as string;
				this.codeGet = JSON.parse(abc);
				console.log("atercomplete code", this.codeGet)
	...
	doSubmit(): void {
		...
		this.mainApiService.postData(appConfig.base_url_slug + method, formData, 2).then(response => {
			if (response.status == 200 || response.status == 201) {
				...
				this.router.navigateByUrl('/main/collection');
				this.isLoading = false;
			}
			else {
				this.isLoading = false;
	...
	onFileSelect(event: any) {
		if (event.controlName == 'image') {
			...
			this.errorMsglogo = 'Please select  image'
		}
	}
