...
@Component({
	selector: 'app-interest_tag-list',
	templateUrl: './interest_tag-list.component.html'
})
export class InterestTagListComponent implements OnInit {
	search: string;
	...
	addNew() {
		this.router.navigateByUrl('main/interest_tag/add');
	}

	toggleView(item: any) {
		console.log(item.status);
		if (item.status) {
			this.tag_status = 'active';
		}
		else {
			this.tag_status = 'not_active';
		}
		var data = {
			status: this.tag_status,
			id: item.id
		}
		var url = 'editInterestTag';
...
let dialogRef = this.dialog.open(AlertDialog, {autoFocus: false});
	...
	gerAccessCodesList(index: any, isLoaderHidden?: boolean): void {
		...
		let url = 'viewInterestTag?page=' + index + '&per_page=' + this.perPage + '&sort_order=Desc';
	...
	onEdit(interest_tag: any): void {
		localStorage.setItem('Interest_Tag', JSON.stringify(interest_tag));
		this.router.navigateByUrl('main/interest_tag/' + interest_tag.id);
	}
