...
import { ChangeOutletImage } from './outlets-change-image.component';
import { appConfig } from '../../../config';
import { AlertDialog } from '../../lib';
@Component({
    selector: 'outlet-image',
    templateUrl: './outlets-images.component.html'
})
export class outletsImage{
    sub: Subscription;
    id: any;
    type: any;
    Form: FormGroup;
    ...
    resolutionOut: number;
    resolutionTrue: boolean;
    ...
    constructor(private _route: ActivatedRoute, protected formbuilder: FormBuilder,
        ...
        this.resolutionOut = 0;
        this.resolutionTrue = false;
            ...
            .then(result => {
                if (result.status == 200 && result.data) {
                    ...
                    result.data.outlets.forEach(element => {
                        this.menudata = element.outletImages;
                        this.menudatatype = element.outletImages[0]?.type;
                    });
    ...
    for(let i = 0 ; i < filesAmount ; i++)
    {
        ...
        const Img = new Image ();
        const fileToUpload = (event.target.files[i]);
        ...
        Img.onload = (e : any) =>{
            ...
            if (filesAmount > 0 && filesAmount < 6) {
                ...
                this.resolutionTrue = true;
                if(this.resolutionOut < 6){
                    this.resolutionOut += 1;
                }
            }
            }
            else{
...
// }
    ...
    doSubmit() {
        ...
        if (this.id != 'add') {
            this.isLoading = true;
            let method = '';
            method = 'updateOutletMultipleImages';
            ...
            // }
            if (this.type == 'image' && this.images.length > 0) {
    ...
    onEditArrangment() {
        if (this.imagesarray.length > 0) {
            ...
            let dialogRef = this.dialog.open(ChangeOutletImage, { autoFocus: false, data: { pageValue: this.imagesarray, outlet: this.id } });
